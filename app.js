...
/* pokemonları ekleyeceğimiz için fetch pokemon yazdık. asenkron olduğu için async ardından arrow function actık for ile yukarıda yazdığımız 151 adet pokemonu ifade eden pokemon_count'ı elirttik.  i(yani 1 151den küçüktür dedik, ve her seferinde 1 arttır dedik, ardından await getPokemon yazdık) */
const fetchPokemons = async () => {
  for (let i = 1; i <= pokemon_count; i++) {
    await getPokemon(i)
  }
}
/* await ile gelen getpokemonu değişken olarak atadık yine async açtık () e id yazdık, sonrasında const url ile fotografı çektik yukarıda parantez içerisine yazdıgımız id yi $ ile tanımladık. */
